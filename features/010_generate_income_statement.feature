Feature: Generate Income Statement
  ...
  Background:
    Given I am logged in
      ...
      And a ledger_item exists with total_amount: "-2.99", currency: "USD", account: account "Bank A/C", sender: contact "Self", recipient: contact "Starbucks", match: match "m1"
      And a ledger_item exists with total_amount: "2.99", currency: "USD", account: account "Coffee", sender: contact "Starbucks", recipient: contact "Self", match: match "m1"
      And a match "m2" exists
      And a ledger_item exists with total_amount: "100", currency: "USD", account: account "Bank A/C", sender: contact "Self", recipient: contact "Customer", match: match "m2"
